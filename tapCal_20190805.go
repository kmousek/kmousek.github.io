...
func CalculChargeAmount(stub shim.ChaincodeStubInterface, tapRd *jsonStruct.TapRecord, recordMemory jsonStruct.RecordMemory ) (jsonStruct.TapCalculreturnValue, error) {
	...
	var subContract jsonStruct.Contract   //계약 서브 구조체 (past와 current중 하나 Agreement매핑)
	...
	subContract = searchAgtIdx(actContract, nowDate)
	fmt.Println(bCommitmentFlag)


	//return구조체 값 매핑
	stTapCalcReturn.ContractID = subContract.CONTID
	stTapCalcReturn.Peoriod[0] = subContract.ContDtl.CONTSTDATE
	stTapCalcReturn.Peoriod[1] = subContract.ContDtl.CONTEXPDATE
	stTapCalcReturn.Currency = subContract.ContDtl.CONTCURCD
	...
	for i:=0;i<len(subContract.ContDtl.CalcBas);i++{
		if tapRd.CdrInfos.CALL_TYPE_ID == subContract.ContDtl.CalcBas[i].CALLTYPECD {
			stCalcBas = subContract.ContDtl.CalcBas[i]
	...
	for i:=0;i<len(subContract.ContDtl.CalcSpcl);i++{
		if subContract.ContDtl.CalcSpcl[i].MODELTYPECD == gModelTypeImsiCap {
			stSubCtrtCalcSpImsiCap = subContract.ContDtl.CalcSpcl[i]
			bImsiCapFlag = true
		}else{
			bImsiCapFlag = false
		}

		if subContract.ContDtl.CalcSpcl[i].MODELTYPECD == gModelTypeCommit {
			stSubCtrtCalcSpCommit = subContract.ContDtl.CalcSpcl[i]
	...
	*/
	...
	if bImsiCapFlag == true {
		...
		if sScImsiType == gScTypeChRate && bIsMonetary == false{    //change Rate and 사용량 base
			f64ImsiCapCharge, err = calcImsiCapDuration(stub, stCalcBas, stSubCtrtCalcSpImsiCap, tapRd)
			if err != nil{
				//에러처리
				return stTapCalcReturn, errors.New( err.Error())
			}
			...
			f64ImsiCapCharge, err = calcImsiCapMonetary(stub, stCalcBas, stSubCtrtCalcSpImsiCap, tapRd)
			if err != nil{
				//에러처리
				return stTapCalcReturn, errors.New( err.Error())
			}
	...
	/*
		if bCommitmentFlag == true {
			// 사용량 base인지 금액 base인지 체크
			if subContract.Commitment.THRSUNIT == gUnitByte || subContract.Commitment.THRSUNIT == gUnitKbyte || subContract.Commitment.THRSUNIT == gUnitMbyte ||
				subContract.Commitment.THRSUNIT == gUnitSec  || subContract.Commitment.THRSUNIT == gUnitMin {
				//사용량 base check,,,,
				bIsMonetary = false
			}else{
				//금액 base check,,,,
			...
			if sScCommitType == gScTypeChRate && bIsMonetary == false{    //change Rate and 사용량 base
				f64CommitCharge, err = calcCommitDuration(stub, &subContract, tapRd)
				if err != nil{
					//에러처리
					return stTapCalcReturn, errors.New( err.Error())
				}
				...
				// perImsi 금액 기준으로 check
				f64CommitCharge = calcCommitMonetary(stub, &subContract, tapRd)
			}
		}
	*/
...
/************************************************************************************************************/
func calcImsiCapMonetary(stub shim.ChaincodeStubInterface, stCalcBas jsonStruct.CalcBas, stCalcSpcl jsonStruct.CalcSpcl, tapRd *jsonStruct.TapRecord) (float64, error) {
...
/************************************************************************************************************/
func calcImsiCapDuration(stub shim.ChaincodeStubInterface, stCalcBas jsonStruct.CalcBas, stCalcSpcl jsonStruct.CalcSpcl, tapRd *jsonStruct.TapRecord) (float64, error) {
